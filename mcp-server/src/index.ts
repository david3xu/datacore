...
import { appendEvent, getBronzeDir, searchEvents, getTasks } from './bronze-store.js';
import { McpServer, StdioServerTransport, z } from './runtime-deps.js';
...
server.tool(
  'get_tasks',
  'Query task board from Bronze events. Returns active tasks by default. Use task_id to get full history of a specific task. Tasks carry three layers of context: WHY (problem, impact), WHERE (project, stage, constraints), HOW (pattern, lessons, acceptance criteria).',
  {
    status: z
      .enum(['active', 'completed', 'failed', 'all'])
      .optional()
      .describe("Filter: 'active' (default), 'completed', 'failed', 'all'"),
    assigned_to: z.string().optional().describe('Filter by assignee'),
    task_type: z.string().optional().describe('Filter by task type'),
    task_id: z.string().optional().describe('Get full event history for a specific task'),
  ...
  async ({ status, assigned_to, task_type, task_id, limit }) => {
    ...
    if (result.mode === 'history') {
      ...
      const contextSummary = [
        ...
        ctx.acceptance && `Acceptance: ${(ctx.acceptance as string[]).join('; ')}`,
        ctx.lessons && `Lessons: ${(ctx.lessons as string[]).join('; ')}`,
